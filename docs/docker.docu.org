...
** TODO explain /docker/ teminology: Image, Container, Repository, Registry, Index.
** Installation
*** Kernel options
...
As /Docker/ is under heavy developement, the file system storing /Docker/ related informatian changes rapidly. The main directory to look for /Docker/ relevant bits and bytes is /var/lib/docker/. In this section *GUID* is the full blown container id as given by =docker ps -a -no-trunc=.
*** LXC configuration
...
*** Container Volumes
If a container mounts a volume from inside the files on that volue are stored under /var/lib/docker/vfs/dir/GUID/. Data stored under these volumes are persistend between container runs. There is a way to share these volumes between containers. 
