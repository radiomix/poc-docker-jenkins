...
As docker is under heavy developement, the file system storing docker related informatian changes rapidly. The main directory to look for docker relevant bits and bytes is /var/lib/docker/. 
*** Configuration 
...
*** LXC configuration
Using the Linux Container package [[/lxc/][http://linuxcontainers.org/]], docker configures each container by setting lxc options in /var/lib/docker/container/GUID/config.lxc/. Here GUID is the full blown container id as given by =docker ps -a -no-trunc=  
*** Contianer Root File System
The corresponding root file system is stored in /var/lib/docker/devicemapper/mnt/GUID/.
