...
This chapter deals with installation issues and some basic docker commands. It is mainly take from the \href{http://docs.docer.io/en/latest/}{Docker documentation}. 
\subsection{\textbf{TODO} explain docker teminology: Image, Container, Repository, Registry, Index.}
\label{sec-2-1}
...
As docker is under heavy developement, the file system storing docker related informatian changes rapidly. The main directory to look for docker relevant bits and bytes is \emph{var/lib/docker}. 
...
Using the Linux Container package \href{file://./lxc/}{http://linuxcontainers.org/}, docker configures each container by setting lxc options in \emph{var/lib/docker/container/GUID/config.lxc}. Here GUID is the full blown container id as given by \texttt{docker ps -a -no-trunc}  
\subsubsection{Contianer Root File System}
\label{sec-2-5-3}

The corresponding root file system is stored in \emph{var/lib/docker/devicemapper/mnt/GUID}.
