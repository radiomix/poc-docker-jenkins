...
\begin{itemize}
\item Get the base image
\begin{itemize}
\item out of the registry or
\item imported as a tar file
\end{itemize}
\item Build a \emph{Docker} container with the source code and tag it
\begin{itemize}
\item push it into the registry or
\item export it as a tar file
\end{itemize}
\item Start the newly tagged container on the production server
\end{itemize}
\subsection{Get the base image}
\label{sec-6-1}

We use the \texttt{FROM ubuntu/scala-base} statement within a  Dockerfile to indicate the proper base image. If no registry server is used, we import the base image by typing \texttt{cat foo.bar.tar | sudo docker import repos/user:tag}.
\subsection{Build the container}
\label{sec-6-2}

The same Dockerfile is used to build the container with \emph{tomcat7} and the \emph{Scala} WebApp installed. After pulling the source code from a \emph{git} repo we compile it and copy the war-file into \emph{tomcat7} webapp directory. \emph{Docker} expexts the build process to take place in a context, which in our case is the directory containing the proper Dockerfile. We then type:
...
Within the same directory we double the build process resulting in just another tag. We then push the image into a registry or export it as a file. 
...
\end{verbatim}
The second command pushes the image into the registry. If we omit the version part it is automatically tagged \texttt{latest}.
We provide a Dockerfile  \hyperref[Dockerfile]{file:image-builds/scala/Dockerfile} to build the \emph{Scala} WebApp from a fresh cloned git repository. The alternative way is to   
\subsubsection{Export the image as a file}
\label{sec-6-3-2}


\begin{verbatim}
jenkins: $ sudo docker build -rm -t registry.local:5000/ubuntu-12-04/scala-ver-x
jenkins: $ sudo docker export registry.local:5000/ubuntu-12-04/scala-ver-x > ubuntu-12-04.scala-ver-x.tar
\end{verbatim}
We could also export a container after we adjusted it manually.
\subsection{Start the container}
\label{sec-6-4}

On our production machine we get the latest image, stop the old one and start the new one, exposing port 8080. Again we have two ways of doing so. Using a registry server we want to
\subsubsection{Pull the image and start it}
\label{sec-6-4-1}


\begin{verbatim}
production: $ sudo docker pull  registry.local:5000/ubuntu-12-04/scala-ver-x
production: $ sudo docker stop  registry.local:5000/ubuntu-12-04/scala-ver-y
production: $ sudo docker run -d -p :8080:8080 -i -t  registry.local:5000/ubuntu-12-04/scala-ver-x
\end{verbatim}
Again, omitting the version part would result in pulling the latest image. Without a registry server we just 
\subsubsection{Import the image and start it}
\label{sec-6-4-2}

To import the image from a local file we replace the first line in the above example with the import command. 

\begin{verbatim}
production: $ cat ubuntu-12-04.scala-ver-x.tar |  sudo docker import  registry.local:5000/ubuntu-12-04/scala-ver-x
production: $ sudo docker stop  registry.local:5000/ubuntu-12-04/scala-ver-y
production: $ sudo docker run -d -p :8080:8080 -i -t  registry.local:5000/ubuntu-12-04/scala-ver-x
\end{verbatim}
\section{AWS}
