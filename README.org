...
** Building a private repo
As [[http://blog.docker.io/2013/07/how-to-use-your-own-registry/][Sam Alba]], a Dotcloud.io engineer since the first beginning of /docker/ describes, we can build a registry container, provided =gunicorn= and =pip= is installed:
#+BEGIN_SRC sh
#install pip 
...
** Changes to the repository building code
The code posted by Sam Alba did not work out of the box neither on a Debian Wheezy (7.4) nor on an Ubuntu 12.4. We had to previously install and upgrade these packages to get it running:
*** Upgrade /pip/ 
#+BEGIN_SRC sh
...
*** Web Services 
[[https://quay.io/][quay.io]] serves private registry the web, [[https://index.docker.io/][docker.io]] provides a public index server.
