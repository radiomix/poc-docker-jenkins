...
#+END_SRC 
Note the =variables= section, defining three variables that we do not want to store in the json file for security reasons. Suppose the above json file is called docker.json, we start the build process typing:
#+BEGIN_SRC sh
