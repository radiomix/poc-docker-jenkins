...
Each time the WebApp has changed, it will be committed into a /git/ repository. Via a web hook, /Jenkins/ get's triggered to build a container with the committed source code:
 ...
 - Build a /Docker/ container with the source code and tag it 
  - push it into the registry or
  - export it as a tar file
 - Start the newly tagged container on the production server
** Get the base image
We use the =FROM ubuntu/scala-base= statement within a  Dockerfile to indicate the proper base image. If no registry server is used, we import the base image by typing =cat foo.bar.tar | sudo docker import repos/user:tag=.
** Build the container
The same Dockerfile is used to build the container with /tomcat7/ and the /Scala/ WebApp installed. After pulling the source code from a /git/ repo we compile it and copy the war-file into /tomcat7/ webapp directory. /Docker/ expexts the build process to take place in a context, which in our case is the directory containing the proper Dockerfile. We then type:
#+BEGIN_SRC sh
...
#+END_SRC
The second command pushes the image into the registry. If we omit the version part it is automatically tagged =latest=.
We provide a Dockerfile  [[Dockerfile][file:image-builds/scala/Dockerfile]] to build the /Scala/ WebApp from a fresh cloned git repository. The alternative way is to   
