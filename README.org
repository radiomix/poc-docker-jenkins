...
=======
*** Save an image
Anothger way to persist and move a docker installation is to save a container. Docker hereby will pack the layers and metadata of all the chain required to build the image. We do not loose the build history of the image.
#+BEGIN_SRC sh
sudo docker save mkl/debian:7.4 > mkl-debian-7.4.tar
#+END_SRC
*** Load an image
We can now load the image from a local file into docker, getting all the image build history restored.
#+BEGIN_SRC sh
sudo docker loag < mkl-debian-7.4.tar 
#+END_SRC
The difference in exporting to saving and importing to loading is towfold. The later works on images instead of containers and tars the meta data together with the image enabeling later rollback to an earlear image version.
