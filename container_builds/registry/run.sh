...
AWS_OPTION=" -e AWS_REGION=$AWS_REGION -e AWS_BUCKET=$AWS_BUCKET -e STORAGE_PATH=$STORAGE_PATH  -e AWS_KEY=$AWS_KEY -e AWS_SECRET=$AWS_SECRET -e SEARCH_BACKEND=$SEARCH_BACKEND "
...
docker run --name $CONTAINER_NAME $REGISTRY_SETTINGS_FLAVOR $AWS_OPTION $REGISTRY_CONFIG_OPTION $REGISTRY_IMAGE
